...
class LiveCompliance:
    """Incremental counterpart to :func:`compliance` for a live video stream.

    ``observed`` is a set of ``(track_id, violation)`` pairs positively
    indicated by an associated ``NO-*`` detection.  A new event is emitted
    only after the configured on-delay; it is then closed after the off-delay
    (or when its track disappears).  Callers own persistence/snapshots so a
    single snapshot can be taken exactly when an event is opened.
    """

    def __init__(self, config: dict | None = None):
        config = config or {}
        self.on_delay = float(config.get("on_delay_seconds", ON_DELAY_SECONDS))
        self.off_delay = float(config.get("off_delay_seconds", OFF_DELAY_SECONDS))
        self.cooldown = float(config.get("cooldown_seconds", COOLDOWN_SECONDS))
        self.track_loss = float(config.get("track_loss_close_seconds", TRACK_LOSS_CLOSE_SECONDS))
        self.states: dict[tuple[int, str], dict] = {}
        self.last_seen: dict[int, float] = {}
        self.recent_closed: dict[tuple[int, str], tuple[dict, float]] = {}
        self.next_event_id = 0

    def _open(self, key: tuple[int, str], timestamp: float) -> tuple[dict, bool]:
        old = self.recent_closed.get(key)
        if old and timestamp - old[1] <= self.cooldown:
            event = old[0]
            event.update(end=None, status="open", reopens=event["reopens"] + 1)
            del self.recent_closed[key]
            return event, False
        self.next_event_id += 1
        event = {
            "event_id": self.next_event_id, "track_id": key[0], "violation": key[1],
            "start": round(timestamp, 3), "end": None, "status": "open",
            "reopens": 0, "snapshot": None,
        }
        return event, True

    def _close(self, key: tuple[int, str], state: dict, timestamp: float, status: str, changes: list[dict]) -> None:
        event = state.get("event")
        if event is None:
            return
        event.update(end=round(timestamp, 3), status=status)
        self.recent_closed[key] = (event, timestamp)
        state.update(status="clear", since=timestamp, event=None)
        changes.append({"kind": "closed", "event": event})

    def advance(self, timestamp: float, tracks_present: set[int], observed: set[tuple[int, str]]) -> list[dict]:
        """Advance one frame and return ``opened``/``closed`` event changes."""
        changes: list[dict] = []
        keys = set(self.states) | observed
        for track_id in tracks_present:
            self.last_seen[track_id] = timestamp
        for key in keys:
            track_id, _ = key
            state = self.states.setdefault(key, {"status": "clear", "since": timestamp, "event": None})
            if track_id not in tracks_present:
                if timestamp - self.last_seen.get(track_id, timestamp) >= self.track_loss:
                    self._close(key, state, timestamp, "closed_track_lost", changes)
                continue
            active = key in observed
            if state["status"] == "clear":
                if active:
                    state.update(status="pending", since=timestamp)
            elif state["status"] == "pending":
                if not active:
                    state.update(status="clear", since=timestamp)
                elif timestamp - state["since"] >= self.on_delay:
                    event, fresh = self._open(key, state["since"])
                    state.update(status="open", since=timestamp, event=event)
                    changes.append({"kind": "opened", "event": event, "fresh": fresh})
            elif state["status"] == "open":
                if not active:
                    state.update(status="clearing", since=timestamp)
            elif state["status"] == "clearing":
                if active:
                    state.update(status="open", since=timestamp)
                elif timestamp - state["since"] >= self.off_delay:
                    self._close(key, state, timestamp, "closed_normal", changes)
        return changes
