...
def resolve_person_id(model: YOLO, subject: str) -> int:
    ...
    Exported checkpoints commonly spell the COCO class ``person`` while the
    PPE vocabulary uses ``Person``; class lookup should not disable tracking
    merely because of case/spacing punctuation.
    """
    by_name = {name: idx for idx, name in model.names.items()}
    if subject in by_name:
        return by_name[subject]
    normalise = lambda value: "".join(ch for ch in str(value).lower() if ch.isalnum())
    by_normalised = {normalise(name): idx for name, idx in by_name.items()}
    if normalise(subject) not in by_normalised:
        raise SystemExit(f"vocabulary subject '{subject}' not in model classes: {sorted(by_name)}")
    return by_normalised[normalise(subject)]
